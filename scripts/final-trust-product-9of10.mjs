...
function trustEn(page = 'site') {
  ...
  return `<!-- final-trust:start --><section class="section evaluation-confidence"><div class="container"><div class="section-head"><span class="kicker">Evaluation confidence</span><h2>Evaluate the product workflow with clear commercial and documentation context.</h2><p>${intro}</p></div><div class="evaluation-grid">${card('Transparent routes', 'Pricing, trial, demo, contact, privacy, terms and DPA pages are visible before a team commits.')}${card('Product workflow first', 'Teams can review projects, welds, inspections, WPS/WPQ context, evidence, documents and dossier preparation before checkout.')}${card('Standards context stays careful', 'WeldInspect Pro supports documentation workflows around relevant standards. Official standard texts, certification, qualified review and formal conformity decisions remain leading.')}${card('Connected product review', 'Ask for a guided walkthrough to see project setup, weld records, inspection evidence, dossier readiness and handover output together.')}</div></div></section><!-- final-trust:end -->`;
}
