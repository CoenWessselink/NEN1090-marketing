...
function pricingDepth(lang = 'en') {
  const nl = lang === 'nl';
  const faqs = nl
    ? [['Kan ik starten met een proefperiode?', 'Ja. Vraag een proefperiode aan om de productworkflow te beoordelen voordat u een betaald abonnement kiest.'], ['Heb ik een creditcard nodig?', 'Nee. Voor een proefperiode of demo is geen creditcard nodig. Mollie wordt alleen gebruikt voor een gekozen betaald abonnement.'], ['Kunnen we later gebruikers toevoegen?', 'Ja. Teamgrootte en uitrol kunnen later opnieuw worden afgestemd.'], ['Wat gebeurt er na betaling?', 'De betaalpagina bevestigt bedrag, btw en cyclus voordat Mollie de betaling verwerkt. Daarna volgt account- en onboardingopvolging.'], ['Is implementatieondersteuning mogelijk?', 'Ja. Inrichting, rollen en implementatie kunnen tijdens demo of contact worden besproken.'], ['Kunnen we eerst een demo aanvragen?', 'Ja. De demo laat projecten, lassen, inspecties, bewijs, rapportage en dossieropbouw in samenhang zien.'], ['Wat als ons team een afwijkende inrichting nodig heeft?', 'Gebruik demo of contact om teamcontext, privacy, implementatie en commerciële eisen te bespreken.']]
    : [['Can I start with a trial?', 'Yes. Request trial access to evaluate the product workflow before choosing a paid route.'], ['Do I need a credit card?', 'No credit card is needed for a trial or demo. Mollie checkout is only used for a selected paid plan.'], ['Can we add more users later?', 'Yes. Team size and rollout scope can be reviewed later.'], ['What happens after payment?', 'Checkout confirms amount, VAT and billing cycle before Mollie processes payment. Account and onboarding follow-up comes next.'], ['Is implementation support available?', 'Yes. Setup, roles and implementation can be discussed through demo or contact.'], ['Can we request a demo first?', 'Yes. The demo connects projects, welds, inspections, evidence, reporting and dossier preparation.'], ['What if our team needs a custom setup?', 'Use demo or contact to discuss team context, privacy, implementation and commercial requirements.']];
  return section(`pricing-${lang}`, `<section class="completion-band pricing-depth"><div class="container"><div class="section-head"><span class="kicker">${nl ? 'Prijzen met context' : 'Pricing with context'}</span><h2>${nl ? 'Kies tussen proefperiode, demo, betaling of maatwerk.' : 'Choose trial, demo, payment or custom setup with clear expectations.'}</h2><p>${nl ? 'De pagina maakt duidelijk wat inbegrepen is, wat na betaling gebeurt en wanneer contact of demo verstandiger is.' : 'The page explains what is included, what happens after payment and when contact or demo is the better route.'}</p></div><div class="completion-faq">${faqs.map(([q, a]) => `<article><h3>${q}</h3><p>${a}</p></article>`).join('')}</div></div></section>`);
}
